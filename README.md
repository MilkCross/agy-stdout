...
本スクリプトは v1.0.6 以降を前提とし、**プロセスの自然終了**を待ってから DB を読み出す方式を採用しています。これによりツール呼び出しを行うモデルでも、ツールチェーン完了後の最終応答を正確に取得できます。
...
- スクリプト内のパスは **Windows 向け**に設定されています。Unix/Linux/macOS で使用する場合は、スクリプト冒頭の以下の 2 箇所を環境に合わせて変更してください。

  ```python
  # Windows（デフォルト）
  CONVERSATIONS_DIR = os.path.expandvars(r"%USERPROFILE%\.gemini\antigravity-cli\conversations")
  AGY_EXE = os.path.expandvars(r"%LOCALAPPDATA%\agy\bin\agy.exe")

  # Unix/Linux/macOS の場合の例
  CONVERSATIONS_DIR = os.path.expanduser("~/.gemini/antigravity-cli/conversations")
  AGY_EXE = "/usr/local/bin/agy"  # インストール先に応じて変更
  ```
